...
if(isset($_POST['pbutton']))
{
	...
	$title=$_POST['ptitle'];	
 	...
 	$data=mysqli_connect("localhost","root","","blog") or die(); 
	$db=mysqli_query($data,"UPDATE post SET `title`='$title' WHERE `pid`='$notrec'");
	$db=mysqli_query($data,"UPDATE post SET `content`='$pst' WHERE `pid`='$notrec'");
...
<li><a class='waves-effect waves-light' href='blogger.php'>Blogger page</a></li>
...
$notrec=$_REQUEST["notrec"];
$data=mysqli_connect("localhost","root","","blog") or die();
$nm=mysqli_query($data,"SELECT `emailid`,`title`,`content` FROM post WHERE `pid`= '$notrec'");
$nm=mysqli_fetch_assoc($nm);
$content=$nm['content'];
$title=$nm['title'];
echo "<form  class=sharebar method=post action=editblog.php?notrec=$notrec>
Title:<br><textarea  cols=30 rows=1 name=ptitle id=tbar>$title</textarea><br>
Content:<textarea name=postupdate cols=60 rows=5 class=postbar>$content</textarea>
