...
if(isset($_POST['givep']))
{
 	$notrec=$_REQUEST["notrec"];
	$data=mysqli_connect("localhost","root","","blog") or die(); 
	$db=mysqli_query($data,"UPDATE signin SET `permission`='1' WHERE  `emailid`='$notrec' ");
}
if(isset($_POST['removep']))
{
 	$notrec=$_REQUEST["notrec"];
	$data=mysqli_connect("localhost","root","","blog") or die(); 
	$db=mysqli_query($data,"UPDATE signin SET `permission`='0' WHERE  `emailid`='$notrec'");
}
if(isset($_POST['makea']))
{
 	$notrec=$_REQUEST["notrec"];
	$data=mysqli_connect("localhost","root","","blog") or die(); 
	$db=mysqli_query($data,"UPDATE signin SET `admin`='1' WHERE  `emailid`='$notrec'");
}
if(isset($_POST['removea']))
{
 	$notrec=$_REQUEST["notrec"];
	$data=mysqli_connect("localhost","root","","blog") or die(); 
	$db=mysqli_query($data,"UPDATE signin SET `admin`='0' WHERE  `emailid`='$notrec'");
}
...
<li><a class='waves-effect waves-light' ><?php echo "Hello, ".$name; ?></a></li>
 ...
 <li><a class='waves-effect waves-light' href='admincontact.php'>Contact Requests</a></li>
...
<div class='post-start row' id='post_po_summary5405969120918013030'>
	...
	<?php                   
	$data=mysqli_connect("localhost","root","","blog") or die();
	$db=mysqli_query($data,"SELECT `emailid`,`fname`,`lname`,`permission`,`admin` FROM signin WHERE `emailid` != '$usid'");
	...
	foreach ($db as $var ) {
		...
		if($var['permission']=='0')
		{	
			echo $name."<br><br><form method=post action=perm.php?notrec=$notrec ><input type=submit id=sbtn1 value='Give Writing Permission' name=givep></form>
		  <br>";

		}else{
			echo $name."<br><br><form method=post action=perm.php?notrec=$notrec ><input type=submit id=sbtn1 value='Remove Writing Permission' name=removep></form>
		  <br>";
		}
		if($var['admin']=='0')
		{	
		echo  " <form method=post action=perm.php?notrec=$notrec ><input type=submit id=sbtn1 value='Make Admin' name=makea></form>
		  <br>";
		} else
		{
			echo  " <form method=post action=perm.php?notrec=$notrec ><input type=submit id=sbtn1 value='Remove From Admin' name=removea></form>
		  <br>";
		}
	}
