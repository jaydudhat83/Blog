...
{
	$db=mysqli_query($data,"SELECT `permission`  FROM signin WHERE `emailid`='$usid'");
	$db=mysqli_fetch_assoc($db);
	if($db['permission']=='0')
	{	
		echo "<script type='text/javascript'>alert('You Are not Authorized To Post!!');</script>";	 
...
if(isset($_POST['premove']))
{
 	$notrec=$_REQUEST["notrec"];
	$data=mysqli_connect("localhost","root","","blog") or die(); 
	$db=mysqli_query($data,"DELETE FROM post WHERE  `pid`='$notrec'");
}
if(isset($_POST['plike']))
{
	$notrec=$_REQUEST["notrec"];
	$flag=0;
	$db=mysqli_query($data,"SELECT `emailid` FROM plike WHERE  `pid`='$notrec'");
  	foreach ($db as $id) {
		$a=$id['emailid'];
		if($a==$usid)
		{
	...
	mysqli_query($data,"INSERT INTO plike VALUES(' ','$notrec','$usid')");
 	}else
 	{
 		$db=mysqli_query($data,"DELETE FROM plike WHERE  `pid`='$notrec'");
...
if(isset($_POST['pcom']))
{
 	...
 	$content=$_POST['ccon'];
	$data=mysqli_connect("localhost","root","","blog") or die(); 
	mysqli_query($data,"INSERT INTO comment VALUES(' ','$notrec','$usid','$content')");
}
...
<li><a class='waves-effect waves-light' href='index.php'>Viewer page</a></li>
...
<h4>Write Post</h4>
<form  class="sharebar" method="post" action="blogger.php">
<input type="test" name="ptitle" placeholder="Title..." id="tbar" >
<br>
<textarea name="postupdate" cols="60" rows="5" placeholder="Write Something Here" class="postbar"></textarea> 
<input type="submit" name="pbutton" id="pbtn" value="Post"></form>
</div>
</div>
</div>	
...
<?php                   
$data=mysqli_connect("localhost","root","","blog") or die();
$var=mysqli_query($data,"SELECT `pid`,`emailid`,`title`,`content`,`time` FROM post WHERE `emailid`= '$usid' ORDER BY `pid` desc");
...
$ccom=$nm["clike"];
 	...
 	$flag=0;
	$db=mysqli_query($data,"SELECT `emailid` FROM plike WHERE `pid`='$pid'");
  	foreach ($db as $id) {
		$a=$id['emailid'];
		if($a==$usid)
		{
...
<form class=pform1 method=post action=editblog.php?notrec=$pid><input type=submit id=sbtn2 value='Edit' name=ped></form>
<form class=pform2 method=post action=blogger.php?notrec=$pid><input type=submit id=sbtn2 value='Remove' name=premove></form></div>
...
<form  method=post action=blogger.php?notrec=$pid><input type=submit id=sbtn3 value='$lvalue' name=plike></form>
<form class=pform2 method=post action=blogger.php?notrec=$pid>
<input type=text name=ccon placeholder='Comment here...' id=sbar1>
<input type=submit id=sbtn3 value='Comment' name=pcom>
</form>
