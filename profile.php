...
if(isset($_POST['notwin']))
{
	$re=$_REQUEST['rec'];
	$data=mysqli_connect("localhost","root","","blog") or die();
	$db=mysqli_query($data,"SELECT `sender` FROM request WHERE `receiver`='$re' AND `sender`='$usid'");
	$db=mysqli_fetch_assoc($db);
	if($db["sender"]==$usid){
		echo "<script type='text/javascript'>alert('Already Added to Notification List');</script>";
	}
	else{
	mysqli_query($data,"INSERT INTO request VALUES(' ','$usid','$re')");
	echo "<script type='text/javascript'>alert('Added to Notification List');</script>";
}
} 
...
$data=mysqli_connect("localhost","root","","blog") or die();
$nm=mysqli_query($data,"SELECT `fname`,`lname`,`emailid` FROM signin WHERE `emailid`='$re'"); 
...
<h2 class='post-home-title post-main-home-title'>$fn $ln</h2>
 
 
 Email:$userid <br><br>
 Add to Notification Window<br>
 <form method=post action=profile.php?rec=$userid><input type=submit id=sbtn value=Click Window name=notwin></form> 
</div>
</div>
</div>";
