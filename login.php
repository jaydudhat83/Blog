 ...
 if(isset($_GET["signupbtn"]))
{
	...
	elseif (!preg_match("/^[a-zA-Z ]*$/",$liname)) {
		 echo "<script type='text/javascript'>alert('Only letters and white space allowed in name');</script>";
	}
	...
	$db=mysqli_query($data,"SELECT `emailid` FROM signin");
  	foreach ($db as $id) {
		$a=$id['emailid'];
		if($a==$mail)
		{
	...
	if($flag==0){
		mysqli_query($data,"INSERT INTO veri VALUES(' ','$finame','$liname','$mail','$passwd','0')");
		
		header("location:email.php?id=".$mail);
...
if(isset($_GET["signinbtn"]))
{
	...
	if($flag==1){
		$db=mysqli_query($data,"SELECT `password` FROM signin WHERE `emailid`='$ids'");
		...
		echo "<script type='text/javascript'>alert('Email Not Registered Yet');</script>";
	}
}
                        	...
                        	<div class="form-box">
	                            ...
	                            <div class="form-bottom">
				                    <form role="form" action="login.php " method="get" class="login-form">
				                        ...
				                        <a href="adminlog.php">Go to Admin Login</a><br>
				                        <a href="index.php">Go to Viewer Page</a>
				                    </form>
			                    </div>
		                    </div>
